...
qemu-system-x86_64 -no-reboot -machine type=q35,accel=kvm,smm=off -smp 4 -m 8G -cpu max,pmu=off -nographic -append 'panic=30 selinux=0 audit=0 maxcpus=1 split_lock_detect=off memorizer_enabled_boot=no nokaslr no_hash_pointers loglevel=8 memalloc_size=4 console=ttyS0' -kernel $root/$O/arch/x86/boot/bzImage -initrd initramfs
