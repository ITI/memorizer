 ...
 * Copyright (c) 2024-2026, Board of Trustees of the University of Illinois
...
static inline void __memorizer_kmalloc(unsigned long call_site, const void *ptr,
...
static bool in_memblocks(uintptr_t va_ptr)
{
	int i;
 ...
 * specified PID value. 
 */
static __always_inline bool memorizer_is_enabled(bool filter) {
 ...
 */
__always_inline void memorizer_mem_access(const void* addr, size_t size, bool
		write, uintptr_t ip)
{
	unsigned long flags;
 ...
 *
 */
static void __memorizer_free_kobj(uintptr_t call_site, uintptr_t kobj_ptr)
{
	struct memorizer_kobj *kobj;
	unsigned long flags;
	struct access_from_counts *afc;
 ...
 */
static void memorizer_free_kobj(uintptr_t call_site, uintptr_t kobj_ptr)
{
	if (__memorizer_enter()) {
		track_induced_free();
		return;
...
static inline void __memorizer_kmalloc(unsigned long call_site, const void
...
#if 0
static void memorizer_memblock_free(phys_addr_t base, phys_addr_t size)
{
}
#endif
#if 0
...
static int memstrcmp(const char *cs, const char *ct)
{
	unsigned char c1, c2;
...
static bool is_memorizer_cache_alloc(char * cache_str)
{
...
#if 0
static void memorizer_free_pages_exact (unsigned long call_site, struct page *page, unsigned
		int order)
{
	/*
	 * Condition for ensuring free is from online cpu: see trace point
...
static ssize_t memorizer_write_file_bool(struct file *file, const char __user *user_buf,
